import { Autocomplete, Box, Button, Card, CardContent, CardHeader, Container, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, FormControlLabel, Grid2, IconButton, List, ListItem, ListItemButton, ListItemText, Stack, Switch, Tab, Tabs, TextField, Typography } from "@mui/material"
...
function EditUnitsMode() {
  ...
  const nameConflict = units.some(u => u.name === editing.name && u.id !== editing.id);
...
function CombatTab() {
  ...
  const nameConflict = combats.some(c => c.name === selected.name && c.id !== selected.id);
